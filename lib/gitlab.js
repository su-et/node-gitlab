...
module.exports = function gitlab (url, token, sudoUser) {
  ...
  // check that url and token are both strings
  ...
  // GitLab admins can use the SUDO header
  // to act as other users
  if (sudoUser) {
      ...
      r(path, function (err, resp, body) {
        if (err) {
          ...
          if (links && links.next) {
            ...
            apiRequest(path, opts, results, links)
            .then(fulfill)
            .catch(reject);
          } else {
